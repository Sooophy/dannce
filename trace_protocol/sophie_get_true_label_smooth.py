...
com_file = 'DANNCE/predict_results/twd5/com3d_used.mat'
...
for i in range(22):
    for j in range(3):
        smoothed_pred[:,j,i] = signal.savgol_filter(np.squeeze(true_pred[:,j,i]), window_length=17, polyorder=3, deriv=0, delta=1.0, axis=- 1, mode='interp', cval=0.0)
        # smoothed_pred[:,j,i] = signal.medfilt(np.squeeze(smoothed_pred[:,j,i]), kernel_size=11)
        smoothed_pred[:,j,i] = signal.medfilt(np.squeeze(smoothed_pred[:,j,i]), kernel_size=5)

# print(smoothed_pred.shape)
smoothed_pred_file = 'smoothed_prediction_twd5_medfilt5.mat'
...
sio.savemat(smoothed_pred_file, mdict)
